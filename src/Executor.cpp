    ...
    return [apState, apEx, apOut](
        ...
        if (apState->FileOk() == true) {
            apEx->AddTask(aThreadNum,
                          MakeReadAndHashLambda(
                              apState, apEx, apOut));
            apState->AddBytesToHash();
        } else {
            apEx->AddTask(aThreadNum,
                          MakeChecksumFinishLambda(
                              apState, apEx, apOut));
        }
    };
}

Task MakeComputeChecksumLambda(
    ...
    return
        [apState, apEx, apOut](uint32_t aThreadNum) {
            ...
            if (apState->ChecksumIsOk()) {
                apOut->NotifyGoodChecksum(
                    apState->GetPath());
            } else {
                apOut->NotifyBadChecksum(
                    apState->GetPath());
            }
    ...
    FileSystem::FindFiles(
        aPath, [&](std::filesystem::path aP) {
            ActivityStarted(); // Matching
                               // ActivityDone() is in
                               // the final Task
            AddTask(compute::MakeComputeChecksumLambda(
                aP, aConfig.mBlockSize, this, apOut));
        });
}
    ...
    aUserIn.ReadChecksumsFromFile(
        ...
        [&](std::filesystem::path aP,
            ...
            ActivityStarted(); // Matching
                               // ActivityDone() is in
                               // the final Task
            AddTask(check::MakeCheckChecksumLambda(
                aP,
                aChecksum,
                aUserIn.mBlockSize,
                this,
...
// ActivityStarted is called below to account for the
...
Executor::Executor(void) : mtRunning{0} {
    ...
    for (int i = 0; i < 4; i++) {
        mlThreads.push_front(
            std::thread(&Executor::Worker, this, i));
    }
}

// Wait for all the work to complete.  If the main
...
void Executor::Wait(void) {
    ...
    while (mtRunning > 0 || !mAnyoneTasks.Empty()) {
        // Make tasklists first b/c vector will
        // realocate and move stuff and cause segfaults
...
void Executor::Worker(uint32_t aThreadNum) {
    ...
    haveTask =
        mvThreadTasks[aThreadNum]->GetTask(newTask);
    if (!haveTask) {
    ...
    while (haveTask || mtRunning) {
        ...
        haveTask = mvThreadTasks[aThreadNum]->GetTask(
            newTask);
        if (!haveTask) {
            haveTask = mAnyoneTasks.GetTask(newTask);
        }
