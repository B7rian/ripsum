...
// Shorthand for a Task.  Tasks have to capture
...
class TaskList {
  public:
    ...
    // GetTask copies a tast to aT and returns true if
    // there is a task to be gotten, otherwise returns
    // false
    bool GetTask(Task &aT);

    // Empty returns true if there are no tasks in the
    // list
    bool Empty(void);
