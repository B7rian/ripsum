...
// I don't know what encoding is used when reading NTFS
// on Linux.
//
// C++ seems to think the encoding is OS-specific but
// it is filesystem-specific and can be different on 1
// OS reading mixed filesystems.
//
// Conversion from UTF-16 is not defined (well?) in
...
void ConsoleOutput::Done(void) {
    if (mBadSums) {
        std::cerr << "sha256sum: WARNING: " << mBadSums
                  << " computed ";
        std::cerr << ((mBadSums == 1) ? "checksum"
                                      : "checksums");
        std::cerr << " did NOT match" << '\n';
    }

    if (mBadLines > 0) {
        std::cerr << "sha256sum: WARNING: "
                  << mBadLines;
        std::cerr << ((mBadLines == 1) ? " line is"
                                       : " lines are");
...
void ConsoleOutput::UserNeedsHelp(void) {
    std::vector<std::string> vHelp = {
        ...
        "",
        "-c, --check\tRead SHA256 sums from the FILEs "
        "and check them",
        "-s <size>\tRead <size> bytes at a time from "
        "the input file(s)",
        "         \tDefault is 8192",
        "--help\tdisplay this help and exit",
        "",
        "Sums are checked and computed using OpenSSL "
        "or whatever libcrypt",
        "is installed on your system. The generated "
        "output is compatible with",
        "sha256sum from GNU coreutils and can be used "
        "interchangably with",
