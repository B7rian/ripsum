...
//
// File is the file-related state that gets passed
...
class File {
  public:
    ...
    // ReadBytes reads some bytes from the file into an
    // internal buffer Returns the number of bytes read
    uint32_t ReadBytes(void);

    // GetBytes provides a pointer to the latest read
    // buffer and returns the number of bytes in the
    // buffer.  This is a bit C-ish and can be made
    // safer.
  ...
  private:
    ...
    uint32_t
        mBlockSize; // Size of a single block of data
    ...
    std::list<Buffer>
        mlBuffers; // A collection of buffers
    std::mutex mBuffersMutex; // Mutex for buffer
                              // collection access
};
